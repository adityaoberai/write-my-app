...
- Never use em dashes (—) or en dashes (–) in any field. Use colons, parentheses, or rewrite the sentence instead.
...
- When naming identifier fields, use plain names like "id", "user_id", "post_id". Never call an id a "document id" or label entities as "documents" anywhere in the spec, even when the backend is a document database.
...
- Never apologize for length, never include filler. The spec is consumed by another AI coding agent and should read like a crisp brief.
